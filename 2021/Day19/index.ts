...
const parse = (dirName: string, fileName: string): Scanner[] => {
  const result: Scanner[] = [];
  let current: Scanner | undefined = undefined;
  let scannerID = 0;
  for (const line of getTextFileLines(dirName, fileName)) {
    if (line.startsWith("--")) {
      current = {
        beacons: [],
        connections: {},
...
const setDistance = (scanners: Scanner[]) => {
  for (const scanner of scanners) {
    ...
    for (let ai = 0; ai < beacons.length; ai++) {
      const beacon1 = beacons[ai];
      for (let bi = ai + 1; bi < beacons.length; bi++) {
        const beacon2 = beacons[bi];
        const dist = distance(beacon1.point, beacon2.point);
        beacon1.dist.push(dist);
        beacon2.dist.push(dist);
      }
    }
  }
};
...
export const part1 = (dirName: string, fileName: string) => {
  const scanners = parse(dirName, fileName);
  setDistance(scanners);
  const p1 = setConnectionsAndCount(scanners);
  return p1;
};

export const part2 = (dirName: string, fileName: string) => {
  const scanners = parse(dirName, fileName);
  setDistance(scanners);
